...
from app.parser import parse_game_url, parse_ratings_history_url, parse_game_log_url, parse_team_ratings_url, parse_ratings_history_dom, parse_ratings_history_dom_rendered, parse_archive_roster_url, parse_archive_roster_url_rendered
from app.ratings import calculate_box_ratings
from app.wis_client import wis_auth_status, get_wis_session
from app.rendered_client import rendered_fetch_status
...
async def import_ratings_history_url_rendered(url: str, session: Session, player_name: str = "", team: str = "") -> dict:
    parsed = await parse_ratings_history_dom_rendered(url)
    ...
    return {
        ...
        "player_id": parsed.get("player_id"),
        "rows": imported,
        "detected_color_cells": parsed.get("detected_color_cells", 0),
        "rendered": True,
    }


async def sync_team_ratings_url_rendered(ratings_url: str, team: str, session: Session) -> dict:
    ...
    for player in parsed["players"]:
        try:
            result = await import_ratings_history_url_rendered(
            ...
            if result["rows"] > 0:
                ...
                detected_color_cells += result.get("detected_color_cells", 0)
        except Exception as exc:
            errors += 1
            print("RENDERED RATINGS SYNC ERROR:", player, exc, flush=True)
...
def sync_diagnostics(request: Request):
    checks = {
        ...
        "rendered_status": rendered_fetch_status(),
    }
    return templates.TemplateResponse(
...
@app.post("/sync-ratings-all-rendered")
async def sync_ratings_all_rendered(session: Session = Depends(get_session)):
    global LAST_RATINGS_SYNC_RESULT
    results = []
    for target in RATINGS_SYNC_TARGETS:
        try:
            result = await sync_team_ratings_url_rendered(target["url"], target["team"], session)
    ...
    print("SYNC RATINGS ALL RENDERED RESULT:", results, flush=True)
...
# include_in_total = False for FT. FT is tracked/displayed but not included in projected player rating.
RATING_META = [
    ("athleticism", "A", True),
    ("speed", "SPD", True),
    ("rebounding", "REB", True),
...
def rating_numeric_value(snapshot, key: str) -> float:
    ...
    if key == "free_throw":
        raw_str = str(raw or "").strip().upper()
...
def infer_color_from_growth(growth: int) -> str:
    # Fallback only when actual WIS color was not captured.
    if growth >= 30:
        return "green"
    if growth >= 20:
        return "blue"
...
def expected_growth_from_start(color: str, start_value: float, rating_key: str) -> float:
    """
    Ceiling model:
      first Season Start value + color rule = projected ceiling

    Current value does NOT determine ceiling. Current only determines realized/remaining growth.
    All categories cap at 100.
    """
    if rating_key == "work_ethic":
        # WE is always black/special. It is not color-potential-capped,
        # but the visible rating still caps at 100.
        return max(0, min(100 - start_value, WE_EXPECTED_GROWTH))
...
def build_potential_summary(rows, color_overrides: dict | None = None):
    ...
    for key, label, include_in_total in RATING_META:
        ...
        realized_growth = current_val - start_val
        ...
        remaining = max(0, projected_peak - current_val)
        developed_pct = (realized_growth / expected_growth * 100) if expected_growth > 0 else 100

        outlier_flag = color == "green" and key in {"perimeter", "low_post"}

        if include_in_total:
            start_total += start_val
            current_total += current_val
            projected_total += projected_peak
            realized_total += max(0, realized_growth)
        ...
        rating_rows.append({
            ...
            "remaining": remaining,
            "developed_pct": max(0, min(999, developed_pct)),
            "outlier_flag": outlier_flag,
        })

    projected_proxy = type("ProjectedSnapshot", (), {})()
    for r in rating_rows:
        setattr(projected_proxy, r["key"], r["projected_peak"])
    ...
    return {
        ...
        "projected_ovr": projected_total,
        "current_ovr": current_total,
        "start_ovr": start_total,
        "development_pct": (realized_total / (projected_total - start_total) * 100) if projected_total > start_total else 100,
...
def build_all_potential_rows(session: Session, team: str = ""):
    ...
    for (player, player_id, snap_team), snap_rows in grouped.items():
        ...
        rows.append({
            ...
            "player_id": player_id,
            "team": snap_team,
            "start_ovr": potential["start_ovr"],
            "current_ovr": potential["current_ovr"],
            "projected_ovr": potential["projected_ovr"],
            "realized_total": potential["realized_total"],
            "remaining_total": potential["remaining_total"],
            "development_pct": potential["development_pct"],
...
def growth_band_from_realized(growth: int, rating_key: str) -> str:
    if rating_key == "work_ethic":
        return "WE-special"
    if growth >= 30:
        return "green-like"
...
def build_growth_samples(session: Session):
    snapshots = session.exec(select(PlayerRatingSnapshot)).all()
    grouped = {}
    for snap in snapshots:
    ...
    for player_id, rows in grouped.items():
        ...
        try:
            seasons_span = int(current.season or 0) - int(start.season or 0) + 1
        except Exception:
            seasons_span = 0

        for key, label in GROWTH_RATING_KEYS:
            start_val = _rating_num(getattr(start, key, 0))
            current_val = _rating_num(getattr(current, key, 0))
            growth = current_val - start_val
            band = growth_band_from_realized(growth, key)

            samples.append({
                "player_id": player_id,
                "player": start.player or current.player,
                "team": start.team or current.team,
                "rating_key": key,
                "rating": label,
                ...
                "growth": growth,
                "band": band,
                "start_we": _rating_num(getattr(start, "work_ethic", 0)),
                "current_we": _rating_num(getattr(current, "work_ethic", 0)),
            })

    return samples


def summarize_growth_samples(samples):
    groups = {}
    for s in samples:
        key = (s["rating"], s["band"])
        groups.setdefault(key, []).append(s["growth"])

    summary = []
    for (rating, band), vals in groups.items():
        vals = sorted(vals)
        n = len(vals)
    ...
    order = {label: i for i, (_key, label) in enumerate(GROWTH_RATING_KEYS)}
    band_order = {"green-like": 0, "blue-like": 1, "black-like": 2, "yellow-like": 3, "red-like": 4, "WE-special": 5}
    summary.sort(key=lambda r: (order.get(r["rating"], 99), band_order.get(r["band"], 99)))
    return summary


@app.get("/growth-model", response_class=HTMLResponse)
def growth_model_dashboard(request: Request, session: Session = Depends(get_session)):
    samples = build_growth_samples(session)
    summary = summarize_growth_samples(samples)
    player_count = len({s["player_id"] for s in samples})

    # Show player/rating rows with largest realized growth first.
    top_growth = sorted(samples, key=lambda s: s["growth"], reverse=True)[:100]
...
async def archive_import(
    url: str = Form(...),
    team: str = Form(""),
    use_rendered: bool = Form(True),
    session: Session = Depends(get_session),
):
    try:
        if use_rendered:
            parsed = await parse_archive_roster_url_rendered(url)
        else:
            parsed = parse_archive_roster_url(url)
    except Exception as exc:
        print("ARCHIVE ROSTER LOAD ERROR:", {"url": url, "rendered": use_rendered, "error": str(exc)}, flush=True)
        # Do not hard-crash the page; redirect to growth model after logging.
        return RedirectResponse("/archive?error=load_failed", status_code=303)
    ...
    for player in parsed["players"]:
        try:
            # Use existing reliable ratings import. If rendered ratings eventually proves useful,
            # we can swap this to import_ratings_history_url_rendered.
            result = import_ratings_history_url(
                player["ratings_history_url"],
                session,
                player_name=player.get("player") or "",
            ...
            errors += 1
            print("ARCHIVE RATINGS IMPORT ERROR:", player, exc, flush=True)

    print("ARCHIVE IMPORT RESULT:", {
        "source": url,
        "rendered": parsed.get("rendered", False),
        "players_found": len(parsed["players"]),
        "imported_players": imported_players,
        "imported_rows": imported_rows,
        "errors": errors,
    ...
    return RedirectResponse("/growth-model", status_code=303)
...
async def potential_debug(player_id: str, request: Request, tid: str = "", rendered: bool = True, session: Session = Depends(get_session)):
    existing = session.exec(
    ...
    parsed = await parse_ratings_history_dom_rendered(url) if rendered else parse_ratings_history_dom(url)
