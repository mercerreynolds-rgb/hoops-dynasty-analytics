...
def is_my_team_win(game: Game) -> bool:
    if game.away_team == MY_TEAM:
        return (game.away_score or 0) > (game.home_score or 0)
    if game.home_team == MY_TEAM:
        return (game.home_score or 0) > (game.away_score or 0)
    return False


def opponent_coach(game: Game) -> str | None:
    if game.away_team == MY_TEAM:
...
def opponent_control_type(game: Game) -> str:
    return "Sim AI" if (opponent_coach(game) or "").strip() == "Sim AI" else "Human"
...
def build_form_and_split_rows(stats, games):
    my_games = [g for g in games if g.away_team == MY_TEAM or g.home_team == MY_TEAM]
    my_games_sorted = sorted(my_games, key=lambda g: g.id)

    last5_ids = {g.id for g in my_games_sorted[-5:]}
    win_ids = {g.id for g in my_games if is_my_team_win(g)}
    loss_ids = {g.id for g in my_games if g.id not in win_ids}
    human_ids = {g.id for g in my_games if opponent_control_type(g) == "Human"}
    sim_ids = {g.id for g in my_games if opponent_control_type(g) == "Sim AI"}
...
def build_opponent_control_summary(stats, games):
    my_games = [g for g in games if g.away_team == MY_TEAM or g.home_team == MY_TEAM]
    human_ids = {g.id for g in my_games if opponent_control_type(g) == "Human"}
    sim_ids = {g.id for g in my_games if opponent_control_type(g) == "Sim AI"}
...
def season_dashboard(request: Request, session: Session = Depends(get_session)):
    ...
    form_rows = build_form_and_split_rows(stats, games)
    control_summary = build_opponent_control_summary(stats, games)
