...
async def parse_archive_roster_url_rendered(url: str) -> dict:
    """
    Browser-authenticated archive roster crawler.

    Uses Playwright-rendered DOM instead of raw requests. This is needed because
    some WIS archive pages redirect raw requests to the newer Clerk/OAuth sign-in flow.
    """
    from app.rendered_client import get_rendered_html

    html = await get_rendered_html(url, wait_selector="body")
    soup = BeautifulSoup(html, "lxml")
    lines = clean_lines_from_html(html)
    ...
    def add_player(pid: str, name: str = "", tid: str | None = None, href: str = ""):
        ...
        if pid in players_by_pid:
            if name and (not players_by_pid[pid]["player"] or players_by_pid[pid]["player"] == pid):
                players_by_pid[pid]["player"] = name
            if tid and not players_by_pid[pid].get("tid"):
                players_by_pid[pid]["tid"] = tid
                players_by_pid[pid]["ratings_history_url"] = f"https://www.whatifsports.com/hd/PlayerProfile/RatingsHistory.aspx?tid={tid}&pid={pid}"
            return
    ...
    # 2. Any pid/onclick/data attributes in rendered DOM.
    for tag in soup.find_all(True):
        attrs_raw = " ".join(str(v) for v in tag.attrs.values())
        pid_match = re.search(r"pid=(\d+)", attrs_raw)
        if pid_match:
            text = " ".join(tag.get_text(" ", strip=True).split())
            tid_match = re.search(r"tid=(\d+)", attrs_raw)
            add_player(pid_match.group(1), text, tid_match.group(1) if tid_match else None, "")
    ...
    print("ARCHIVE ROSTER RENDERED PARSER DEBUG:", {
        "url": url,
        "players": len(players),
        "sample": players[:5],
